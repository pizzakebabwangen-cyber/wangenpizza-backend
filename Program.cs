...
// SmarterASP: app in www/admin, Razor views often live in sibling www/views (see Category, Contact, …).
builder.Services.Configure<RazorViewEngineOptions>(options =>
{
    options.ViewLocationFormats.Insert(0, "../views/{1}/{0}.cshtml");
    options.ViewLocationFormats.Insert(0, "../Views/{1}/{0}.cshtml");
});
    ...
    endpoints.MapFallback(async context =>
    {
        ...
        if (!string.IsNullOrEmpty(marker) && host.Contains(marker, StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (path.StartsWith("/api", StringComparison.OrdinalIgnoreCase))
            {
                ...
                await context.Response.WriteAsync("API route not found.");
                return;
            }

...
namespace WangenPizza.Controllers
{
    public class OrderController : Controller
    {
		...
		public async Task<IActionResult> Pos()
		{
			...
			// Also fetch orders that are acknowledged (in any tracking status)
			var acknowledgedOrders = await context.Orders
				.AsNoTracking()
				.Where(o => o.PosAcknowledged && (o.OrderStatus == null || o.OrderStatus != "Delivered"))
				.Include(o => o.OrderItems!)
					.ThenInclude(oi => oi.Product)
				.Include(o => o.OrderItems!)
					.ThenInclude(oi => oi.ExtensionOrderItem)
			...
			var allOrders = data.Union(acknowledgedOrders).OrderByDescending(o => o.Id).ToList();
			...
			return View("PosStepper", allOrders);
		}
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            OrderDetailsDto model = new OrderDetailsDto()
            {
                ...
                Notes = data.Notes,
                Items = (List<Models.OrderItem>)data.OrderItems,
		...
		public async Task<IActionResult> AcknowledgePosOrder([FromForm] int id, [FromForm] int minutes)
		{
			...
			var rows = await context.Database.ExecuteSqlInterpolatedAsync(
				$"UPDATE Orders SET PosAcknowledged = CAST(1 AS bit), PreparationMinutesEmailed = {minutes}, OrderStatus = 'Preparing', StatusUpdatedAt = GETUTCDATE() WHERE Id = {id} AND PosAcknowledged = CAST(0 AS bit)");
			if (rows != 1)
			{
				TempData["PosMessage"] =

...
namespace WangenPizza.Services
{
    public class ExtensionService: IExtensionService
    {
        ...
        {
            return await context.Extension.Where(e => e.Kind == "MainExtension").Include(a => a.Category).OrderBy(e => e.DisplayOrder).ThenBy(e => e.Name).ToListAsync();
        }
        public async Task<IEnumerable<Extension>> GetByCategoryId(int categoryId)
        {
            return await context.Extension.Where(a => a.CategoryId == categoryId).OrderBy(a => a.DisplayOrder).ThenBy(a => a.Name).ToListAsync();
        }

    ...
    public class UserService : IUserService
    {
		...
		public async Task<UserManagerResponse> RegisterUserAsync(RegisterModel model)
		{
			...
			if(olduser !=null)
			{
                ...
                {
                    var resendResult = await SendActivationEmailAsync(olduser);
            ...
            if (result.Succeeded)
            {
                ...
                if (!roleresult.Succeeded)
                {
                    return new UserManagerResponse
                    {
                        Message = "Benutzerrolle konnte nicht gesetzt werden",
                        IsSuccess = false,
                        Errors = roleresult.Errors.Select(e => e.Description)
                    };
                }

                var emailResult = await SendActivationEmailAsync(user);
        ...
        private async Task<UserManagerResponse> SendActivationEmailAsync(ApplicationUser user)
        {
            try
            {
            ...
            catch
            {
                return new UserManagerResponse
                {
                    Message = "Bestätigungs-E-Mail konnte nicht gesendet werden. Bitte später erneut versuchen.",
                    IsSuccess = false,
                };
            }
        }
            ...
            public async Task<UserManagerResponse> ConfirmEmailAsync(string userId, string token)
        {
            ...
            try
            {
                ...
                normalToken = Encoding.UTF8.GetString(decodedToken);
            }
            catch
            {
                return new UserManagerResponse
		...
		public async Task<UserManagerResponse> ForgetPasswordAsync(string email)
		{
            ...
            string url = $"{frontend}/resetPassword?email={encodedEmail}&token={encodedToken}";

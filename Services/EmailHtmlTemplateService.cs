...
namespace WangenPizza.Services
{
    public class EmailHtmlTemplateService : IEmailHtmlTemplateService
    {
        ...
        public async Task<string> GetActvateEmailTemplate(ApplicationUser user)
        {
            ...
            // Kunden-Link: bevorzugt öffentliche Domain (pizzawangen.ch), falls dieselbe App dort /api ausliefert.
            // Sonst in appsettings.json "EmailConfirmationBaseUrl" auf die API-Basis setzen (z. B. https://admin.pizzawangen.ch).
            var confirmationBase = GetEmailConfirmationBaseUrl();
            var uid = Uri.EscapeDataString(user.Id);
            var tok = Uri.EscapeDataString(validEmailToken);
            string url = $"{confirmationBase}/api/Auth/confirm-email?userId={uid}&token={tok}";
        ...
        {
            var configured = _configuration["EmailConfirmationBaseUrl"]?.Trim();
            ...
            return "https://admin.pizzawangen.ch";
        }

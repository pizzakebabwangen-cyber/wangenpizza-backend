...
namespace WangenPizza.Api_s.Controllers
{
    ...
    public class CompanyController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAvailableOrderTimes()
        {
            ...
            var dateOptions = new List<OrderDateOptionDto>();

            // Heute + morgen + übermorgen: Vorbestellung bleibt bewusst kurz und planbar.
            for (var dayOffset = 0; dayOffset <= 2; dayOffset++)
            {
                var targetDate = now.Date.AddDays(dayOffset);
                var (from1, till1, from2, till2) = GetScheduleForDay(company, targetDate.DayOfWeek);
                var timeOptions = new List<OrderTimeOptionDto>();
                var startTime = dayOffset == 0 ? now.AddMinutes(45) : targetDate;

                AddTimeRange(timeOptions, from1, till1, targetDate, startTime);
                AddTimeRange(timeOptions, from2, till2, targetDate, startTime);

                if (timeOptions.Count == 0) continue;

                dateOptions.Add(new OrderDateOptionDto
                {
                    Date = targetDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture),
                    Label = FormatOrderDateLabel(targetDate, dayOffset),
                    Options = timeOptions
                });
            }
            ...
            if (dateOptions.Count == 0)
            {
                return Ok(new
                {
                    ...
                    dateOptions = new List<OrderDateOptionDto>()
                });
            }
            ...
            return Ok(new
            {
                ...
                options = dateOptions[0].Options,
                dateOptions
            });
        }

        private static (string? from1, string? till1, string? from2, string? till2) GetScheduleForDay(CompanyData company, DayOfWeek dayOfWeek)
        {
            return dayOfWeek switch
            {
                DayOfWeek.Monday => (company.MondayFrom1, company.MondayTill1, company.MondayFrom2, company.MondayTill2),
                DayOfWeek.Tuesday => (company.TuesdayFrom1, company.TuesdayTill1, company.TuesdayFrom2, company.TuesdayTill2),
                DayOfWeek.Wednesday => (company.WednesdayFrom1, company.WednesdayTill1, company.WednesdayFrom2, company.WednesdayTill2),
                DayOfWeek.Thursday => (company.ThursdayFrom1, company.ThursdayTill1, company.ThursdayFrom2, company.ThursdayTill2),
                DayOfWeek.Friday => (company.FridayFrom1, company.FridayTill1, company.FridayFrom2, company.FridayTill2),
                DayOfWeek.Saturday => (company.SaturdayFrom1, company.SaturdayTill1, company.SaturdayFrom2, company.SaturdayTill2),
                DayOfWeek.Sunday => (company.SundayFrom1, company.SundayTill1, company.SundayFrom2, company.SundayTill2),
                _ => (null, null, null, null)
            };
        }

        private static string FormatOrderDateLabel(DateTime date, int dayOffset)
        {
            var suffix = dayOffset == 0
                ? "Heute"
                : dayOffset == 1
                    ? "Morgen"
                    : new CultureInfo("de-CH").DateTimeFormat.GetDayName(date.DayOfWeek);

            return $"{date:dd.MM.yyyy} - {suffix}";
        }

        private bool AddTimeRange(List<OrderTimeOptionDto> timeOptions, string? f, string? t, DateTime targetDate, DateTime startTime)
        {
            ...
            var lastTime = targetDate.Add(till);
            var forbiddenTime = lastTime.AddMinutes(-45);

            if (startTime > forbiddenTime) return false;

            var current = targetDate.Add(from);
            if (current < startTime) current = RoundUpToQuarterHour(startTime);
            else current = RoundUpToQuarterHour(current);
        ...
        {
            var remainder = value.Minute % 15;
            if (remainder == 0 && value.Second == 0 && value.Millisecond == 0)
                return value;

            var minutesToAdd = remainder == 0 ? 15 : 15 - remainder;
            return value
        ...
        public class OrderDateOptionDto
        {
            public string Date { get; set; } = string.Empty;
            public string Label { get; set; } = string.Empty;
            public List<OrderTimeOptionDto> Options { get; set; } = new();
        }
        #endregion
